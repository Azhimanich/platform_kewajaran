...
def export_mysql_to_parquet():
    ...
    try:
        ...
        if df.empty:
            print("[Warning] Database kosong. Tidak ada yang di-export.")
            return
            
        print(f"[OK] Berhasil membaca {len(df)} baris data.")
        
        # Konversi ke parquet
        output_file = "data.parquet"
        print(f" Menyimpan dan mengkompresi data ke {output_file}...")
